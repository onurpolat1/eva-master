...
import eva.ml_access.ml_functions as ml_func
...
end_times  = data_shifted.index[config.init_train_period:] # end of training periods (expanding or sliding window)
...
# ---------------------------------------------------------------
if not data_shifted.index[-1] in end_times:  # force data end point to be included
    end_times = np.hstack((end_times, data_shifted.index[-1]))
...
if len(end_times) > 1 and config.fixed_start == False:
    ...
    if int(np.where(data_shifted.index == end_times[-1])[0] - \
           np.where(data_shifted.index == end_times[-2])[0]) < 19:
...
if config.do_model_fit == True:
    ...
    projections = np.zeros((104+config.horizon, len(col_name))) * np.nan
    projections[:104, 0] = data_shifted[config.target].values

    # time index column for projections
    if config.time_var == 'rangeL':
        proj_index = np.arange(data_shifted.index[0], data_shifted.index[-1] + config.horizon + 1)
        # time range for projection period in quarters
    proj_index = pd.date_range(data_shifted.index[0], periods=112 + config.horizon, freq='Q')
    proj_index = np.char.array(proj_index.year) + b'Q' + np.char.array(proj_index.quarter)
    ...
    for t, end in enumerate(end_times):
        i_t = int(np.where(proj_index == str(end))[0])
        ...
        if config.fixed_start == False and t > 0:
            ...
            start = data_shifted.index[i_s]
        else:
            i_s = 0
            start = data_shifted.index[i_s]

        # training data
        df_train = data_func.data_framer(data_shifted, config.target, config.features, \
        ...
        if not config.CV_name == None:
            ...
            # cross-validation
            if do_CV == True:
                print ("\t\t{0} values between {1} and {2}.".format(len(config.CV_values), config.CV_values[0],
                                                             config.CV_values[-1]))
        ...
        if t == 0:
            ...
            if not train_L == data.M:
                ...
                X_VAR = data.data_shifted[[config.target] + config.features].iloc[train_L - config.horizon:train_L,
                        :].values
                ...
                for k in range(config.horizon):
                    ...
                    # VAR(1) reference
                    if not config.ref_model == None:
                        VAR_fcast = VAR_model.forecast(X_VAR[k].reshape((1, len(config.features) + 1)), config.horizon)[
                            -1, 0]
                        if config.is_class == False:
        ...
        if i_t_hor + 1 <= data.M:  # error summary
            print ("actual: {0},\n\t\t\ttest errors: point ({1}), mean to horizon ({2})".format(y_val, point_err,
                                                                                         np.round(mean_test_err, 3)))
        if i_t_hor + 1 == data.M:
            print ("\n\tFuture Projections:\n")
