...
def ML_train_tester(df, target, features, method, m_test=1, n_boot=500, is_class=False, is_zero_one=False, \
    ...
    """
        ...
        feat_weights, test_indices = np.zeros((n_boot, n_col - 1)), np.zeros(
            (n_boot, m))  # weights for feature importance, test_index over bootstraps
        ...
        for t in range(n_boot):
            ...
            if not method == 'VAR':  # VAR part of statsmodels library (treated differently)
                ML = model_selection(method, n_HN=n_col - 1, CV_name=CV_name,
                                     CV_value=CV_value)  # n_HN only used for neural network
                ...
                if CV_name == None:
                    model = ML.fit(maxlags=1)  # model fit, defaults VAR with one lag
                else:
                    exec('model = ML.fit(' + CV_name + '=' + str(CV_value) + ')')
   ...
   data : numpy.array, optional (Default value = None)
    ...
    # select model
        ...
        if method == 'NN-rgr':  # ONLY WORKING with scikit-learn >= 0.18
            ...
            if CV_name == None:
                model = skl_nn.MLPRegressor(hidden_layer_sizes=(n_HN, n_HN), alpha=.001, activation='relu',
                                            solver='lbfgs')
            else:
            ...
            # create and train network
            if CV_name == None:
                model = skl_nn.MLPClassifier(hidden_layer_sizes=n_HN, alpha=2., activation='logistic', solver='lbfgs')
            else:
            ...
            if CV_name == None:
                model = skl_tree.DecisionTreeRegressor(max_features='sqrt', max_depth=5)
            else:
                exec(
                    'model = skl_tree.DecisionTreeRegressor(' + CV_name + '=' + str(CV_value) + ',max_features="sqrt")')
        elif method == 'Tree-clf':
            if CV_name == None:
                model = skl_tree.DecisionTreeClassifier(max_depth=8)
            else:
                exec('model = skl_tree.DecisionTreeClassifier(' + CV_name + '=' + str(
                    CV_value) + ')')  # e.g. 'max_depth'
        elif method == 'Forest-rgr':
            if CV_name == None:
                model = skl_ens.RandomForestRegressor(200, max_features='sqrt', max_depth=11)  # 7 best for CPI
            else:
                exec('model = skl_ens.RandomForestRegressor(200,' + CV_name + '=' + str(CV_value) + ',max_depth=11)')
        elif method == 'Forest-clf':
            if CV_name == None:
                model = skl_ens.RandomForestClassifier(200, max_depth=9, \
                                                       criterion='entropy', max_features='sqrt')
            else:
                exec('model = skl_ens.RandomForestClassifier(200,' + CV_name + '=' + str(
                    CV_value) + ',criterion="entropy")')
        elif method == 'SVM-rgr':
            if CV_name == None:
                model = skl_svm.SVR(C=50, gamma=0.001, epsilon=0.2)
            else:
                exec('model = skl_svm.SVR(' + CV_name + '=' + str(
                    CV_value) + ',epsilon=0.2,gamma=0.001)')  # change for gamma/epsilon CV
        elif method == 'SVM-clf':
            if CV_name == None:
                model = skl_svm.SVC(C=1e3, gamma=1)
            else:
                exec('model = skl_svm.SVC(C=1e3,' + CV_name + '=' + str(CV_value) + ')')
        elif method == 'kNN-rgr':
            if CV_name == None:
                model = skl_neigh.KNeighborsRegressor(n_neighbors=2, p=1)
            else:
                exec('model = skl_neigh.KNeighborsRegressor(' + CV_name + '=' + str(
                    CV_value) + ',p=1)')  # e.g. 'n_neighbors'
        elif method == 'kNN-clf':
            if CV_name == None:
                model = skl_neigh.KNeighborsClassifier(n_neighbors=5, p=1)
            else:
                exec('model = skl_neigh.KNeighborsClassifier(n_neighbors=3,' + CV_name + '=' + str(
                    CV_value) + ')')  # e.g. 'n_neighbors'
        elif method == 'NB-clf':
            if CV_name == None:
                model = skl_NB.GaussianNB()
            else:
                exec('model = skl_NB.MultinomialNB(' + CV_name + '=' + str(
                    CV_value) + ')')  # e.g. 'alpha' smoothing parameter (0 for no smoothing).
        elif method == 'OLS':
            if CV_name == None:
            ...
            if CV_name == None:
                ...
                exec("model = skl_lin.logistic.LogisticRegression(" + CV_name + "=" + str(
                    CV_value) + ",solver='liblinear')")
        elif method == 'VAR':
